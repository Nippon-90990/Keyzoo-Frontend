...
export default function XboxGiftCard() {
    ...
    useEffect(() => {
        async function getRegions() {
            try {
                const res = await fetchFromStrapi(
                    "api/xbox-gift-cards?fields[0]=card_region&pagination[pageSize]=200"
                );
    ...
    useEffect(() => {
        ...
        async function getFilteredProducts() {
            try {
                ...
                const res = await fetchFromStrapi(
                    `api/xbox-gift-cards?populate=*&filters[card_region][$eq]=${encodeURIComponent(
                    ...
                    {products.map((item) => {
                        ...
                        return (
                            <div key={item.id} className='mb-2 mt-2'>
                                {item.Available ? (<Link
                                    href={`/store/category/gift-card/xbox/${item.slug}`}
